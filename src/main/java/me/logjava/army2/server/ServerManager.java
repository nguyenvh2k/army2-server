...
import me.logjava.army2.team.TeamImageOutput;
...
import static me.logjava.army2.server.BangXHManager.bangXHString;
import static me.logjava.army2.server.BangXHManager.bangXHString1;
import static me.logjava.army2.server.User.nvEquipDefault;
...
public class ServerManager {
    ...
    protected static void init() {
        ...
        try {
            ...
            while (res.next()) {
                MapData.MapDataEntry mapEntry = new MapData.MapDataEntry();
        ...
        nvEquipDefault = new NVData.EquipmentEntry[NVData.entrys.size()][5];
        ...
        try {
            ...
            switch (type) {
                ...
                case 5:
                    ...
                    BangXHManager.BangXHEntry[] bxhA = BangXHManager.getBangXH(type, page);
                    ds.writeByte(page);
                    ds.writeUTF(bangXHString1[type]);
                    for (BangXHManager.BangXHEntry bxhA1 : bxhA) {
                        try {
                            int iddb = bxhA1.iddb;
                            ds.writeInt(iddb);
                            ResultSet red;
    ...
    protected static void napTienMessage(User us, Message ms) throws IOException {
        ...
        if (action == 0) {
            ...
            for (int i = 0; i < NapTienData.entrys.size(); i++) {
                NapTienData.NapTienEntry nE = NapTienData.entrys.get(i);
                ds.writeUTF(nE.id);
                ds.writeUTF(nE.info);
                ds.writeUTF(nE.url);
            }
